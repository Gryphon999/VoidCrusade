...
Built with **Phaser 3 + Three.js + TypeScript + Vite**. Everything — art, 3D models, maps, sounds — is generated by code (at runtime, plus a small deterministic build-time bake of colour-grading LUTs); there are no downloaded assets and no backend.

## Quick Start

```bash
npm install
npm run bake       # regenerate build-time art (colour-grading LUTs); the output is committed too
npm run dev        # http://localhost:5173
```

A clean, reproducible build is `npm install && npm run bake && npm run build`.

## Build

```bash
npm run bake           # build-time art: per-map colour-grading LUTs → src/assets/baked/ (deterministic)
...
Battles are rendered in **real-time 3D (Three.js, WebGL2)** under a transparent Phaser layer that
keeps the HUD, health bars, selection rings and minimap. The 3D camera is an orthographic camera
whose projection equals the classic Dawn-of-War-style oblique view, so gameplay (a flat top-down
simulation), picking and every 2D overlay stay pixel-aligned; the simulation never depends on the
renderer. The choice and its evidence are in [docs/graphics-decision.md](docs/graphics-decision.md);
the per-phase log and self-critique in [docs/graphics-log.md](docs/graphics-log.md).

- **Terrain**: displaced height field with noise-shaped cliffs, shelves and shell craters; baked biome
  albedo plus a shader with world-space detail, bump and triplanar rock; outland mountains.
- **Units**: the procedural puppet models as instanced 3D meshes (animations match frame for frame),
  bevelled plates, faction materials (ceramite/steel, wet chitin), rim light, 3D bodies and wrecks.
- **Buildings**: all 36 structures as 3D models; Iron Void builds rise inside scaffolding with a
  welding seam, Horde structures grow; damage darkening and flicker, turret guns tracking targets,
  gates lifting, collapse and 3D rubble.
- **Lighting**: per-map time of day (dusk, noon, night), soft shadow map, dynamic point lights
  (muzzle flashes, explosions, fires, windows, energy cores, obelisks), ground haze, ACES tone
  mapping, per-map LUT grading, bloom, vignette and grain.
- **Effects**: GPU particles (fireballs, sparks, bouncing debris, lit smoke, tracers and trails per
  weapon, muzzle flashes, acid/psy/flame), 3D ground decals (blood, scorch, tracks), ash, embers
  and dust; fog of war as a post effect with smoky drifting edges.
- **Front end**: live 3D planet (continents, lava faults, clouds, night-side fires, atmosphere)
  with a warship group in the menu and on the campaign map; HUD portraits and Encyclopedia
  previews rendered from the real 3D models; battle fly-in and a final shot on the fallen stronghold.

**Settings → Graphics** picks a tier; **Graphics…** opens renderer, resolution scale, FPS counter
and adaptive quality:

| Tier | Shadows | Bloom | MSAA | Max pixel ratio | Point lights | Model detail |
|------|---------|-------|------|-----------------|--------------|--------------|
| Low | off | off | off | 1 | 4 | 0.6 |
| Medium (default) | 2K | on | 4× | 1.5 | 8 | 0.8 |
| High | 2K | on | 4× | 2 | 12 | 1.0 |
| Ultra | 4K | on | 4× | 3 | 16 | 1.25 |

- **Renderer**: Auto (3D when WebGL2 is available), 3D, or Classic 2D (the original sprite renderer).
- **Resolution scale** 50–100 % multiplies the native device pixel ratio (the UI stays crisp).
- **Adaptive quality** steps down one tier after 6 s below 40 FPS and remembers it.
- **Show FPS** prints FPS, tier, draw calls and triangles.

Performance numbers per tier are in [docs/performance.md](docs/performance.md).

### Requirements and troubleshooting

- **WebGL2** is required for 3D (every current desktop browser). Without it the game falls back to
  the 2D renderer automatically.
- **Low frame rate**: lower the tier or the resolution scale, or keep adaptive quality on. On laptops,
  make sure the browser runs on the discrete GPU / "high performance" power profile and that
  hardware acceleration is enabled in the browser settings.
- **Black or flickering battlefield**: usually a GPU driver issue; switch the renderer to Classic 2D.
- **Blurry picture**: check the resolution scale (100 % = native) and the browser zoom (100 %).
...
src/
  ...
  render3d/   Three.js battlefield: Stage3D, Battle3D, Terrain3D, Units3D, Buildings3D, Props3D, Fx3D,
              Decals3D, Lights3D, Fog3D, Post3D (fog of war, LUT, lens), Space3D (menu/campaign), ModelSnap
  assets/baked/  build-time LUTs from `npm run bake`
...
scripts/bake.ts              build-time bake (colour-grading LUTs)
docs/balance.md              balance notes and simulation results
docs/graphics-decision.md    A0 renderer spike and decision
docs/graphics-log.md         per-phase graphics log and self-critique
docs/performance.md          frame cost per quality tier
```

See [TASK.md](TASK.md), [GRAPHICS_TASK.md](GRAPHICS_TASK.md), [CONTENT_TASK.md](CONTENT_TASK.md) and [GRAPHICS_AAA_TASK.md](GRAPHICS_AAA_TASK.md) for the specifications.
